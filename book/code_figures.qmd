...
mismatch_modelf <- readRDS(here("Rdata", "female_fit_m8.rds"))
...
mismatch_modelm <- readRDS(here("Rdata", "male_fit_m8.rds"))
...
bloom_time_draws <- bloom_model %>%
  epred_draws(
    newdata = expand.grid(TIME = seq(min(bloom_dat$TIME), max(bloom_dat$TIME), length.out = 50)),
    re_formula = NA,
    dpar = TRUE
  ) %>%
...
geom_point(shape = 25, stroke = 1, fill = "#bcd2b1",
  ...
  labs(
    subtitle = "\u03B2 = 0.53, 95% CI = [-0.38, 1.47]",
    y = "Bloom onset (days)",
    x = NULL
  ) +
  custom_theme()
...
plot_B <- ggplot(dat_with_residuals_bloom, aes(x =season_year , y = raw_sigma))+
  ...
  labs(subtitle =  "\u03B2 = 0.11, 95% CI = [0.05, 0.18]",
...
mismatch_time_drawsf <- mismatch_modelf %>%
  epred_draws(
    newdata = expand.grid(
      TIME = seq(min(dat_female_mismatch$TIME), max(dat_female_mismatch$TIME), length.out = 50),
...
mismatch_time_drawsm <- mismatch_modelm %>%
  epred_draws(
    newdata = expand.grid(
      TIME = seq(min(dat_male_mismatch$TIME), max(dat_male_mismatch$TIME), length.out = 50),
...
plot_C <- ggplot() +geom_jitter(data = dat_count_mismatch_yearf, 
  ...
  labs(
    subtitle = "\u03B2 = -1.98, 95% CI = [-5.26, 1.35]",
...
plot_D <- ggplot() +geom_jitter(data = dat_count_sigma_year_female, 
  ...
  labs(subtitle = "\u03B2 = 0.02, 95% CI = [0.02, 0.03]",
    y = "Inconsistency (\u03C3)",
    x = NULL
  ) +
  custom_theme() +
...
plot_Cm <- ggplot() +geom_point(data = dat_count_mismatch_yearm,
 aes(x = ori_time, y = X5MISMATCH, size = n), width = 0.2, height = 0,
  ...
  scale_y_continuous(breaks = seq(-90,210, 30), limits = c(-90,210),minor_breaks = seq(-90,210,15),guide = guide_axis(minor.ticks = TRUE)) +
  labs(
    subtitle = "\u03B2 = -1.80, 95% CI = [-5.24, 1.69]",
    y = "Phenological mismatch (days)",
    x = NULL
  ) +
  custom_theme() +
...
plot_Dm <- ggplot() +geom_jitter(data = dat_count_sigma_year_male, 
  ...
  my_x_scale  +
  scale_y_continuous(breaks = seq(0,150, 30), limits = c(0,150),guide = guide_axis(minor.ticks = TRUE)) +
  labs(subtitle = "\u03B2 = 0.03, 95% CI = [0.02, 0.03]",
    y = "Inconsistency (\u03C3)",
    x = NULL
  ) +
  custom_theme() +
...
fledging_time_drawsf <- fledging_modelf %>%
  epred_draws(
    newdata = expand.grid(
      TIME = seq(min(dat_female_fledging$TIME), max(dat_female_fledging$TIME), length.out = 50),
...
fledging_time_drawsm <- fledging_modelm %>%
  epred_draws(
    newdata = expand.grid(
      TIME = seq(min(dat_male_fledging$TIME), max(dat_male_fledging$TIME), length.out = 50),
...
plot_E <- ggplot() +
  ...
  labs(
    subtitle = "\u03B2 = -0.37, 95% CI = [-0.91, -0.04]",
...
plot_F <- ggplot(fledging_time_drawsf, aes(x = actual_time, y = disc)) +
 ...
 my_x_scale  +
  ...
  labs(subtitle = "\u03B2 = -0.01, 95% CI = [-0.02, -0.01]", 
       y = "Consistency (\u03B6)",
       x = NULL) +
  custom_theme()+theme(plot.subtitle = element_text(face="bold"))
...
### Extra code: plot each level of the ordinal variable

```{r}
#| label: figure-1-plot-E-F-females
#| message: false
#| warning: false
#| eval: false
...
# Create plot E
plot_E_full <- ggplot() +
  stat_lineribbon(data = fledging_time_drawsf, aes(x = actual_time, y = .epred, fill = .category), .width = 0.95, alpha = 0.7) +
  geom_point(data = fledging_props_timef, aes(x = ori_time, y = proportion, bg = FLEDS), shape = 21, color = "white", size = 2, alpha = 0.6) +
  scale_fill_manual(values = c("0" = "#E41A1C", "1" = "#377EB8", "2" = "#4DAF4A", "3" = "#984EA3"), name = "Number of Fledglings:") +
  scale_color_manual(values = c("0" = "#E41A1C", "1" = "#377EB8", "2" = "#4DAF4A", "3" = "#984EA3")) +
  my_x_scale   +
  scale_y_continuous(breaks = seq(0,1, 0.2), limits = c(0,1)) +
  labs(
    subtitle = "Number of fledglings",
    y = "",
    x = NULL
  ) +
  custom_theme(show_legend = TRUE)+
  theme(
...
plot_Em <- ggplot() +
  ...
  scale_y_continuous(breaks = seq(0, 1, 0.2), minor_breaks = seq(0,1,0.1), limits = c(0, 1),guide = guide_axis(minor.ticks = TRUE)) +
  labs(
    subtitle = "\u03B2 = -0.40, 95% CI = [-0.96, -0.06]",
    y = "Probability of Nest Success",
    x = NULL
  ) +
  custom_theme()+theme(plot.subtitle = element_text(face="bold"))
...
plot_Fm <- ggplot(fledging_time_drawsm, aes(x = actual_time, y = disc)) +
 ...
 my_x_scale  +
  scale_y_continuous(breaks = seq(0,0.5, 0.1), limits = c(0,0.5),guide = guide_axis(minor.ticks = TRUE)) +
  labs(subtitle = "\u03B2 = -0.02, 95% CI = [-0.02, -0.01]", 
       y = "Consistency (\u03B6)",
       x = NULL) +
  custom_theme()+theme(plot.subtitle = element_text(face="bold"))
...
```{r}
#| eval: false
plot_Em_extra <- ggplot() +
  stat_lineribbon(data = fledging_time_drawsm, aes(x = actual_time, y = .epred, fill = .category), .width = 0.95, alpha = 0.7) +
  geom_point(data = fledging_props_timem, aes(x = ori_time, y = proportion, bg = FLEDS), shape = 21, color = "white", size = 2, alpha = 0.6) +
  scale_fill_manual(values = c("0" = "#E41A1C", "1" = "#377EB8", "2" = "#4DAF4A", "3" = "#984EA3"), name = "Number of Fledglings:") +
  scale_color_manual(values = c("0" = "#E41A1C", "1" = "#377EB8", "2" = "#4DAF4A", "3" = "#984EA3")) +
  my_x_scale   +
  scale_y_continuous(breaks = seq(0,1, 0.2), limits = c(0,1)) +
...
age_draws_female <- mismatch_modelf %>%
  epred_draws(
    newdata = expand.grid(
      AGE = seq(min(dat_female_mismatch$AGE), max(dat_female_mismatch$AGE), length.out = 50),
...
age_draws_male <- mismatch_modelm %>%
  epred_draws(
    newdata = expand.grid(
      AGE = seq(min(dat_male_mismatch$AGE), max(dat_male_mismatch$AGE), length.out = 50),
...
#plot_C \<- ggplot() +geom_jitter(data = dat_count_mismatch_yearf, aes(x = ori_time, y = X5MISMATCH, size = n), width = 0.2, height = 0, shape = 21, alpha = 0.2,stroke=0.1, fill = "#d2bfb8", colour = "#9096b1")+ stat_lineribbon(data = mismatch_time_drawsf, aes(x = actual_time, y = .epred), .width = 0.95, alpha = 0.6, fill = "#a0786e") + stat_lineribbon(data = mismatch_time_drawsf, aes(x = actual_time, y = .epred), fill = NA, color = "black", linewidth = 1) + my_x_scale + scale_y_continuous(breaks = seq(-50,150, 30), limits = c(-50,150)) + labs( subtitle = "\u03B2 = -1.98, 95% CI = \[-5.26, 1.35\]", y = "Phenological mismatch (days)", x = NULL ) + custom_theme() + theme(legend.position = "none")

#plot_D \<- ggplot() +geom_jitter(data = dat_count_sigma_year_female, aes(x = ori_time, y = raw_sigma), width = 0.2, height = 0, shape = 21, alpha = 0.2,stroke=0.1, fill = "#b1ab90", colour = "#b8cbd2",size=1)+ stat_lineribbon(data = mismatch_time_drawsf, aes(x = actual_time, y = sigma), .width = 0.95, alpha = 0.6, fill = "#cbba62") + stat_lineribbon(data = mismatch_time_drawsf, aes(x = actual_time, y = sigma), fill = NA, color = "black", linewidth = 1) + my_x_scale + scale_y_continuous(breaks = seq(0,60, 15), limits = c(0,60)) + labs(subtitle = "\u03B2 = 0.02, 95% CI = \[0.02, 0.03\]", y = "Inconsistency (\u03C3)", x = NULL ) + custom_theme() + theme(legend.position = "none")+theme(plot.subtitle = element_text(face="bold"))
```
...
plot_G <- ggplot() +
  ...
  labs(
    subtitle = "\u03B2 = 0.32, 95% CI = [0.28, 0.36]\n \U03C1 = 0.34, 95% CI = [0.09, 0.60]",
    y = "Phenological mismatch (days)",
    x = "Age (years)"
  ) +
  custom_theme() + 
...
plot_H <- ggplot() +
  ...
  scale_y_continuous(breaks = seq(0, 150, 30), limits = c(0, 150),guide = guide_axis(minor.ticks = TRUE)) +
  labs(
    subtitle = "\u03B2 = 0.01, 95% CI = [-0.01, -0.00]",
    y = "Inconsistency (\u03C3)",
    x = "Age (years)"
  ) +
  custom_theme() +
...
plot_Gm <- ggplot() +
  ...
  scale_y_continuous(breaks = seq(-90, 210, 30),minor_breaks = seq(-90,210,15), limits = c(-90, 210),guide = guide_axis(minor.ticks = TRUE)) +
  labs(
    subtitle = "\u03B2 = 0.27, 95% CI = [0.22, 31]\n \U03C1 = 0.38, 95% CI = [0.13, 0.60]",
    y = "Phenological mismatch (days)",
    x = "Age (years)"
  ) +
  custom_theme() + 
...
plot_Hm <- ggplot() +
  ...
  scale_y_continuous(breaks = seq(0, 150, 30), limits = c(0, 150),guide = guide_axis(minor.ticks = TRUE)) +
  labs(
    subtitle = "\u03B2 = -0.00, 95% CI = [-0.01, 0.00]",
    y = "Inconsistency (\u03C3)",
    x = "Age (years)"
  ) +
  custom_theme() +
...
fledging_age_drawsf <- fledging_modelf %>%
  epred_draws(
    newdata = expand.grid(
      AGE = seq(min(dat_female_fledging$AGE), max(dat_female_fledging$AGE), length.out = 50),
...
fledging_age_drawsm <- fledging_modelm %>%
  epred_draws(
    newdata = expand.grid(
      AGE = seq(min(dat_male_fledging$AGE), max(dat_male_fledging$AGE), length.out = 50),
...
The next chunks build the bottom row of the figure: fledgling probabilities vs. age and fledgling consistency vs. age.
...
plot_fledging_success_age <- ggplot() +
  ...
  labs(
    subtitle = "\u03B2 = -0.06, 95% CI = [-0.13, -0.03]",
    y = "Probability of Nest Success",
    x = "Age (years)"
  ) +
...
plot_fledging_success_agem <- ggplot() +
  ...
  labs(
    subtitle = "\u03B2 = -0.03, 95% CI = [-0.07, -0.01]",
    y = "Probability of Nest Success",
    x = "Age (years)"
  ) +
...
# Figure 4: Fledgling Success vs. Mismatch

This final figure for females shows the direct relationship between phenological mismatch and the probability of fledgling success.
...
mismatch_drawsf <- fledging_modelf %>%
  epred_draws(
    newdata = expand.grid(
      ZX5MISMATCH = seq(min(dat_female_fledging$ZX5MISMATCH), max(dat_female_fledging$ZX5MISMATCH), length.out = 50),
      TIME = mean(dat_female_fledging$TIME),
      AGE = mean(dat_female_fledging$AGE),
      AFR = mean(dat_female_fledging$AFR),
      LONGEVITY = mean(dat_female_fledging$LONGEVITY)
...
# 1. Calculate posterior draws for "success" (fledglings > 0)
fledging_success_drawsf <- mismatch_drawsf %>%
  filter(.category != "0") %>%
  group_by(across(-c(.epred, .category))) %>% 
  summarize(prob_success = sum(.epred), .groups = 'drop')

# 2. Calculate observed binned proportions for "success"
fledging_success_propsf <- dat_female_fledging %>%
  filter(!is.na(X5MISMATCH)) %>%
  mutate(
    SUCCESS = ifelse(FLEDS == "0", "Failure (0)", "Success (1+)")
  ) %>%
  # Count success/failure for each *exact* X5MISMATCH value
  count(X5MISMATCH, SUCCESS) %>%
  tidyr::complete(X5MISMATCH, 
                  SUCCESS = c("Failure (0)", "Success (1+)"), 
                  fill = list(n = 0)) %>%
  # Group by that exact value
  group_by(X5MISMATCH) %>%
  # Calculate proportion of success
  mutate(total_n_year = sum(n),
         proportion = ifelse(total_n_year > 0, n / total_n_year, 0)) %>%
  filter(SUCCESS == "Success (1+)") %>% # Only need to plot the success points
  ungroup()
...
success_mismatch_label_dataf <- fledging_success_drawsf %>%
  filter(actual_mismatch == min(actual_mismatch) | actual_mismatch == max(actual_mismatch)) %>%
  group_by(actual_mismatch) %>%
  summarise(mean_success_perc = mean(prob_success) * 100, .groups = 'drop')

success_mismatch_label_dataf
```
...
mismatch_drawsm <- fledging_modelm %>%
  epred_draws(
    newdata = expand.grid(
      ZX5MISMATCH = seq(min(dat_male_fledging$ZX5MISMATCH), max(dat_male_fledging$ZX5MISMATCH), length.out = 50),
      TIME = mean(dat_male_fledging$TIME),
      AGE = mean(dat_male_fledging$AGE),
      AFR = mean(dat_male_fledging$AFR),
      LONGEVITY = mean(dat_male_fledging$LONGEVITY)
...
# 1. Calculate posterior draws for "success" (fledglings > 0)
fledging_success_drawsm <- mismatch_drawsm %>%
  filter(.category != "0") %>%
  group_by(across(-c(.epred, .category))) %>% 
  summarize(prob_success = sum(.epred), .groups = 'drop')

# 2. Calculate observed binned proportions for "success"
fledging_success_propsm <- dat_male_fledging %>%
  filter(!is.na(X5MISMATCH)) %>%
  mutate(
    SUCCESS = ifelse(FLEDS == "0", "Failure (0)", "Success (1+)")
  ) %>%
...
# 3. Create data frame for min/max mismatch values (for printing)
success_mismatch_label_datam <- fledging_success_drawsm %>%
  filter(actual_mismatch == min(actual_mismatch) | actual_mismatch == max(actual_mismatch)) %>%
  group_by(actual_mismatch) %>%
  summarise(mean_success_perc = mean(prob_success) * 100, .groups = 'drop')


success_mismatch_label_datam
```
:::

Two plots are created: one showing the probability of fledging success versus mismatch, and another showing the consistency of that success versus mismatch

```{r}
#| eval: false
# Assuming ZX5MISMATCH was centered, calculating the mean of the ORIGINAL mismatch variable
# to correctly scale the plot axis (if necessary).
mean_of_original_mismatchf <- mean(dat_female_fledging$X5MISMATCH, na.rm = TRUE)

fledging_mismatch_draws_category <- fledging_modelf %>%
  tidybayes::epred_draws(
    newdata = expand.grid(
      # Predict across the range of the Z-scored predictor
      ZX5MISMATCH = seq(min(dat_female_fledging$ZX5MISMATCH), max(dat_female_fledging$ZX5MISMATCH), length.out = 50),
      # Hold all other continuous covariates at their mean
      TIME = mean(dat_female_fledging$TIME),
      AGE = mean(dat_female_fledging$AGE),
      AFR = mean(dat_female_fledging$AFR),
      LONGEVITY = mean(dat_female_fledging$LONGEVITY)
    ),
    # Exclude random effects (plotting fixed effect only)
    re_formula = NA,
    # Need the dispersion parameter for the category probabilities
    dpar = TRUE
  ) %>%
  # Create a column for the actual, uncentered mismatch value for the plot x-axis
  dplyr::mutate(
    actual_mismatch = ZX5MISMATCH + mean_of_original_mismatchf,
    .category = factor(.category, levels = c("0", "1", "2", "3")) # Ensure categories are factors
  )

# Preview the data structure
head(fledging_mismatch_draws_category)
```

```{r}
#| eval: false
fledging_props_mismatchf <- dat_female_fledging %>%
  dplyr::filter(!is.na(X5MISMATCH)) %>%
  # Count observations for each category at each exact X5MISMATCH value
  dplyr::count(X5MISMATCH, FLEDS) %>%
  # Fill in zero counts for categories that didn't occur
  tidyr::complete(X5MISMATCH,
                  # FIX: Use ordered() instead of factor()
                  FLEDS = ordered(c("0", "1", "2", "3"), levels = c("0", "1", "2", "3")),
                  fill = list(n = 0)) %>%
  # Group by the X5MISMATCH value to calculate proportions
  dplyr::group_by(X5MISMATCH) %>%
  dplyr::mutate(
    total_n_mismatch = sum(n),
    proportion = ifelse(total_n_mismatch > 0, n / total_n_mismatch, 0)
  ) %>%
  dplyr::ungroup() %>%
  # Create a column for the original X5MISMATCH value for the plot x-axis
  dplyr::rename(ori_mismatch = X5MISMATCH)
```

```{r}
#| label: create-fledging-mismatch-categories
#| message: false
#| warning: false
#| eval: false
# Define a custom color palette for the categories
category_colors <- c("0" = "#E41A1C", "1" = "#377EB8", "2" = "#4DAF4A", "3" = "#984EA3")

plot_mismatch_categories <- ggplot() +
  # Use stat_lineribbon for the prediction and uncertainty intervals
  tidybayes::stat_lineribbon(
    data = fledging_mismatch_draws_category, 
    aes(x = actual_mismatch, y = .epred, fill = .category), 
    .width = 0.95, alpha = 0.7
  ) +
  # Add observed data points (optional, but helpful)
  ggplot2::geom_point(
    data = fledging_props_mismatchf, 
    aes(x = ori_mismatch, y = proportion, fill = FLEDS, size = total_n_mismatch),
    shape = 21, color = "black", alpha = 0.6
  ) +
  # Apply the custom color scale
  ggplot2::scale_fill_manual(values = category_colors, name = "Number of Fledglings:") +
  # Set up the x and y axes
  ggplot2::scale_y_continuous(breaks = seq(0, 1, 0.2), limits = c(0, 1)) +
  ggplot2::scale_x_continuous(breaks = seq(-90, 210, by = 30), limits = c(-90, 210)) +
  ...
  # You need to define your custom_theme() function or use a built-in one like theme_classic()
  # custom_theme(show_legend = TRUE) + 
...
print(plot_mismatch_categories)
```

```{r}
#| label: create-fledging-mismatch-categories1
#| message: false
#| warning: false
#| eval: false
category_colors <- c("0" = "#E41A1C", "1" = "#377EB8", "2" = "#4DAF4A", "3" = "#984EA3")
category_alphas <- c("0" = 0.7, "1" = 0.15, "2" = 0.15, "3" = 0.15)

plot_mismatch_categories <- ggplot2::ggplot() +
  tidybayes::stat_lineribbon(
    data = fledging_mismatch_draws_category, 
    aes(x = actual_mismatch, y = .epred, fill = .category, alpha = .category), 
    .width = 0.95
  ) +
  ggplot2::geom_point(
    data = fledging_props_mismatchf, 
    aes(x = ori_mismatch, y = proportion, fill = FLEDS, size = total_n_mismatch, alpha = FLEDS),
    shape = 21, color = "black"
  ) +
  ggplot2::scale_fill_manual(values = category_colors, name = "Number of Fledglings:") +
  ggplot2::scale_alpha_manual(values = category_alphas, guide = "none") +
  ggplot2::scale_y_continuous(breaks = seq(0, 1, 0.2), limits = c(0, 1)) +
  ggplot2::scale_x_continuous(breaks = seq(-90, 210, by = 30), limits = c(-90, 210)) +
  ...
  ggplot2::theme_classic() +
  ggplot2::theme(
    # CHANGE HERE: c(0.9, 0.6) moves it Right (0.9) and Lower (0.6)
...
plot_mismatch_prob_altf <- ggplot() + 
  geom_point(data = fledging_success_propsf, aes(x = X5MISMATCH, y = proportion,size = total_n_year), 
             shape = 22, alpha = 0.4, stroke = 1, fill = "#9dbcbe", colour = "#a68551") +
  stat_lineribbon(data = fledging_success_drawsf, aes(x = actual_mismatch, y = prob_success), 
                  .width = 0.95, alpha = 0.4, fill = "firebrick") +
  stat_lineribbon(data = fledging_success_drawsf, aes(x = actual_mismatch, y = prob_success), 
                  fill = NA, color = "black", linewidth = 1) +
  scale_y_continuous(
    breaks = seq(0, 1, 0.2), 
  ...
  labs(
    subtitle = "\u03B2 = -0.15, 95% CI = [-0.30, -0.07]",
    y = "Probability of Nest Success",
    x = "Phenological mismatch (days)"
  ) +
  custom_theme()+theme(plot.subtitle = element_text(face="bold"))
...
plot_mismatch_prob_altm <- ggplot() + 
  geom_point(data = fledging_success_propsm, aes(x = X5MISMATCH, y = proportion, size = total_n_year), 
             shape = 22, alpha = 0.4, stroke = 1, fill = "#9dbcbe", colour = "#a68551") +
  stat_lineribbon(data = fledging_success_drawsm, aes(x = actual_mismatch, y = prob_success), 
                  .width = 0.95, alpha = 0.4, fill = "firebrick") +
  stat_lineribbon(data = fledging_success_drawsm, aes(x = actual_mismatch, y = prob_success), 
                  fill = NA, color = "black", linewidth = 1) +
  scale_y_continuous(
  ...
  labs(
    subtitle = "\u03B2 = -0.16, 95% CI = [-0.32, -0.07]",
    y = "Probability of Nest Success",
    x = "Phenological mismatch (days)"
  ) +
  custom_theme()+theme(plot.subtitle = element_text(face="bold"))
...
```{r}
#| label: save-fledging-mismatch-figure
#| eval: false

# Save the figure to files
ggsave(filename = here("Plots", "panel_fig_females_fledging_mismatch.pdf"),
       plot = mismatch_figure,
       width = 9, height = 7,
       dpi = 300, device = cairo_pdf)

ggsave(filename = here("Plots", "panel_fig_females_fledging_mismatch.jpg"),
       plot = mismatch_figure,
       width = 9, height = 7,
       dpi = 300)
```
