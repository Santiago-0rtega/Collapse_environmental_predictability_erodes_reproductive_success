...
This section presents the R code used to evaluate model diagnostics and perform model comparisons for the Bayesian analyses reported in the main text. The workflow is divided into two components:

Model diagnostics: automated checks for common fitting issues such as divergent transitions and low Effective Sample Size (ESS).

Model comparison: systematic evaluation of predictive performance using Leave-One-Out Cross-Validation (LOO-CV).

These procedures were applied to the `pheno_mismatch` and `fleds_full` analyses.

::: {.callout-important appearance="simple"}
Note: Diagnostics for bloom onset models were inspected directly from the model summaries because only one model per threshold was fitted.
:::
...
-   Divergences: counts the number of divergent transitions, which signal potential sampling bias.

-   ESS: evaluates bulk and tail Effective Sample Size, flagging parameters with insufficient independent samples (\< 400; @vehtari_rank-normalization_2021).

The results are compiled into a summary table for each model (separated by sex), reporting the outcome of the diagnostic and any problematic parameters
...
This function inventories all available pre-computed LOO objects for a given analysis, producing an organized data frame of models (sex, model name, file path) available for comparison.
...
This function performs model comparison using the `loo` package. Models are grouped by sex, optionally excluding problematic fits, and ranked by their expected log predictive density (ELPD). The output highlights relative model performance, including differences in ELPD, standard errors, and LOO Information Criterion (LOOIC).
...
The following sections apply the above functions to each analysis set. \## Phenological Mismatch (`pheno_mismatch`)
